...
// copied from https://gist.github.com/eriknstevenson/42a2f619128c5e4be200c2314ac98af5
...
init :: proc() -> bool {
	...
	default_font_config.RasterizerDensity = 1.0
	default_font_config.RasterizerMultiply = 1.0
	default_font_config.ExtraSizeScale = 1.0
	default_font_config.SizePixels = 13
	default_font_config.PixelSnapH = true
	default_font_config.GlyphMaxAdvanceX = math.F32_MAX

	imgui.FontAtlas_AddFontDefault(imgui.GetIO().Fonts, &default_font_config)

	return true
}

shutdown :: proc() {
	pio: ^imgui.PlatformIO = imgui.GetPlatformIO()

	texture_datas: []^imgui.TextureData = mem.slice_ptr(pio.Textures.Data, int(pio.Textures.Size))
	for texture_data in texture_datas {
		if texture_data.Status != .Destroyed {
			backend_data := texture_data.BackendUserData
			rl_texture := cast(^rl.Texture)backend_data
			if rl_texture != nil && rl.IsTextureValid(rl_texture^) {
				rl.UnloadTexture(rl_texture^)
				free(rl_texture)
				texture_data.TexID = 0
				texture_data.BackendUserData = nil
			}
			texture_data.Status = .Destroyed
		}
	}
...
new_frame :: proc() {
	...
	if io.DisplaySize.x > 0 && io.DisplaySize.y > 0 {
		io.DisplayFramebufferScale = {
			f32(rl.GetRenderWidth()) / io.DisplaySize.x,
			f32(rl.GetRenderHeight()) / io.DisplaySize.y,
		}
	} else {
...
update_texture :: proc(texture_data: ^imgui.TextureData) {
	switch texture_data.Status {
	case .OK:
	case .Destroyed:
	case .WantCreate:
		rl_image := rl.Image{}
		rl_image.width = texture_data.Width
		rl_image.height = texture_data.Height
		if texture_data.Format == .Alpha8 {
			rl_image.format = rl.PixelFormat.UNCOMPRESSED_GRAYSCALE
		} else {
			rl_image.format = rl.PixelFormat.UNCOMPRESSED_R8G8B8A8
		}
		rl_image.mipmaps = 1
		rl_image.data = imgui.TextureData_GetPixels(texture_data)
		assert(rl.IsImageValid(rl_image), "expected valid image")
		rl_texture: ^rl.Texture = new(rl.Texture)
		texture_data.BackendUserData = cast(rawptr)rl_texture
		rl_texture^ = rl.LoadTextureFromImage(rl_image)
		imgui.TextureData_SetTexID(texture_data, imgui.TextureID(rl_texture.id))
		texture_data.Status = .OK
		assert(rl.IsTextureValid(rl_texture^), "expected valid texture")

	case .WantUpdates:
		rl_texture: ^rl.Texture = cast(^rl.Texture)texture_data.BackendUserData
		if rl_texture == nil {
			break
		}
		rl.UpdateTexture(rl_texture^, imgui.TextureData_GetPixels(texture_data))
		texture_data.Status = .OK
...
render_draw_data :: proc(draw_data: ^imgui.DrawData) {
	...
	if draw_data.Textures != nil {
		texture_datas: []^imgui.TextureData = mem.slice_ptr(
			draw_data.Textures.Data,
			int(draw_data.Textures.Size),
		)
		for texture_data in texture_datas {
			if texture_data.Status != .OK {
	...
	command_lists: []^imgui.DrawList = mem.slice_ptr(
		draw_data.CmdLists.Data,
		int(draw_data.CmdLists.Size),
	)
	for command_list in command_lists {
		cmd_slice: []imgui.DrawCmd = mem.slice_ptr(
...
render_triangles :: proc(
	...
	texture_id: u32 = cast(c.uint)texture_ptr
...
setup_backend :: proc() {
	...
	io.BackendFlags |= {.HasMouseCursors, .RendererHasTextures}
...
@(private)
set_clip_text_callback :: proc "c" (user_data: ^imgui.Context, text: cstring) {
	rl.SetClipboardText(text)
}

@(private)
get_clip_text_callback :: proc "c" (user_data: ^imgui.Context) -> cstring {
